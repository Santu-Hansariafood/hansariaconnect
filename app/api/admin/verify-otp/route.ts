...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!sessionCookie) {
      return NextResponse.json({ success: false, error: "Session missing" }, { status: 403 })
    }
    let payload: { email: string; hash: string; salt: string; exp: number } | null = null
    try {
      payload = JSON.parse(sessionCookie)
    } catch {
      return NextResponse.json({ success: false, error: "Bad session" }, { status: 400 })
    }
    if (!payload || Date.now() > payload.exp) {
      const res = NextResponse.json({ success: false, error: "Expired" }, { status: 403 })
    ...
    if (hash !== payload.hash) {
      const res = NextResponse.json({ success: false, error: "Incorrect code" }, { status: 401 })
      res.cookies.delete("admin_otp_session")
      return res
    }
