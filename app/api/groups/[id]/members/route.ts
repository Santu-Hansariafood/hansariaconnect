...
type LeanProfile = { userId: Types.ObjectId | string; name?: string; photo?: string }
...
const buildDetailedGroup = async (group: GroupLike, isAdmin: boolean) => {
  const members = Array.isArray(group?.members) ? (group.members as IGroupMember[]) : []
  ...
  profiles.forEach((profile) => profileMap.set(String(profile.userId), profile))
...
export async function POST(
  ...
  try {
    ...
    if (!sanitizedMobiles.length) {
      return NextResponse.json({ error: "Provide at least one valid mobile" }, { status: 400 })
    }
    ...
    const mobilesToAdd = sanitizedMobiles.filter((mobile: string) => !existingMobiles.has(mobile))
    if (!mobilesToAdd.length) {
      return NextResponse.json({ error: "All numbers are already members" }, { status: 400 })
    }

    type LeanUser = { _id: Types.ObjectId; mobile: string }
    const users = await User.find({ mobile: { $in: mobilesToAdd } }).lean<LeanUser[]>()
    const userMap = new Map<string, LeanUser>()
    users.forEach((user) => userMap.set(user.mobile, user))

    const missing = mobilesToAdd.filter((mobile: string) => !userMap.has(mobile))
    if (missing.length) {
    ...
    return NextResponse.json(await buildDetailedGroup(group.toObject() as GroupLike, true))
  } catch (error: unknown) {
...
export async function PATCH(
  ...
  try {
    ...
    if (!Types.ObjectId.isValid(memberId) || !role) {
      return NextResponse.json({ error: "Provide valid member id and role" }, { status: 400 })
    }
    ...
    const { group } = adminGroup
    const membersList = group.members as IGroupMember[]
    const targetMember = membersList.find((member) => String(member.userId) === memberId)
    ...
    if (targetMember.role === role) {
      return NextResponse.json({ error: "No changes required" }, { status: 400 })
    }
