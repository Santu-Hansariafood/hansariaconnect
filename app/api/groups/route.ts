...
export async function POST(req: NextRequest) {
  try {
    ...
    const rawList = Array.isArray(body?.memberMobiles)
      ? body.memberMobiles
      : [];
    ...
    if (!cleanedMobiles.length) {
      return NextResponse.json(
        { error: "At least one valid member is required" },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      { group: buildGroupSummary(newGroup.toObject()) },
      { status: 201 },
    );
  } catch (error: unknown) {
